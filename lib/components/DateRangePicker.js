...
var DateRangePicker = function DateRangePicker(props) {
  ...
  React.useEffect(function () {
    if (defaultValue && !insertedStartDate && !insertedStartDate) {
      if ((0, _moment["default"])(defaultValue, "MM/DD/YYYY").isValid()) {
        var data = {
  ...
  var onMonthNavigate = function onMonthNavigate(marker, action) {
    if (marker === MARKERS.FIRST_MONTH) {
      if (defaultRange === false || defaultRange === undefined) {
        ...
        var _firstNew = (0, _dateFns.addMonths)(firstMonth, action);

        if ((0, _dateFns.isBefore)(_firstNew, secondMonth)) setFirstMonth(_firstNew);
      }
    } else {
