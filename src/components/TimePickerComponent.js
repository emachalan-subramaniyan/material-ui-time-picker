...
const styles = theme => ({
  ...
  select: {
    color:
      getContrastRatio(theme.palette.primary.main, theme.palette.common.black) <
      7
...
class TimePickerComponent extends React.Component {
  constructor(props) {
    ...
    const time = props.value || defaultValue;
    this.state = {
      select: "h",
      starthours:
        props.selectedTime && props.selectedTime.starttime
          ? this.getSelectedValue("starthour")
          : this.getinitialState("time"),
      endhours:
        props.selectedTime && props.selectedTime.endtime
          ? this.getSelectedValue("endhour")
          : this.getinitialState("time"),
      startminutes:
        props.selectedTime && props.selectedTime.starttime
          ? this.getSelectedValue("startminute")
          : props.defaultValue ? this.getinitialState("minutes") :time.getMinutes(),
      endminutes:
        props.selectedTime && props.selectedTime.endtime
          ? this.getSelectedValue("endminute")
          : props.defaultValue ? this.getinitialState("minutes") :time.getMinutes(),
      startsession:
        props.selectedTime && props.selectedTime.starttime
          ? this.getSelectedValue("startsession")
          : this.getinitialState("session"),
      endsession:
        props.selectedTime && props.selectedTime.endtime
          ? this.getSelectedValue("endsession")
          : this.getinitialState("session"),
      opendate: false,
      opentime: false,
      startdate:
        props.selectedDate && props.selectedDate.startdate
          ? props.selectedDate.startdate
          : null,
      enddate:
        props.selectedDate && props.selectedDate.enddate
          ? props.selectedDate.enddate
          : null,
      starttime: null,
      endtime: null,
  ...
  getSelectedValue = data => {
    ...
    if (data === "starthour") {
      return Number(
        selectedTime.starttime.substr(0, selectedTime.starttime.indexOf(":"))
      );
    } else if (data === "endhour") {
      return Number(
        selectedTime.endtime.substr(0, selectedTime.endtime.indexOf(":"))
      );
    } else if (data === "startminute") {
      return parseInt(
        selectedTime.starttime.substr(selectedTime.starttime.indexOf(":") + 1)
      );
    } else if (data === "endminute") {
      return parseInt(
        selectedTime.endtime.substr(selectedTime.endtime.indexOf(":") + 1)
      );
  ...
  getinitialState = data => {
    ...
    } else if(data === 'time' && includeTime && defaultValue && moment(defaultValue, "LT").isValid()){
      let time = Number(defaultValue.split(":")[0])
      return time;
    }else if(data === 'minutes' && includeTime && defaultValue && moment(defaultValue, "LT").isValid()){
      ...
      if (data === "time") {
        if (this.props.timemode && parseInt(this.props.timemode) === 24) {
          if (
            ...
            this.props.restrictTime.starttime < time.getHours()
          ) {
          ...
          return parseInt(this.props.timemode) === 12
            ? time.getHours() % 12
            : new Date().getHours();
        }
        ...
        if (this.props.restrictTime) {
          ...
          return parseInt(this.props.timemode) === 12 && time.getHours() >= 12
            ? "PM"
            : "AM";
        }
      }
    ...
    if (data === "start") {
      if (this.state.startsession === "PM") {
        this.setState(
          {
            startsession: "AM",
            starthours: this.props.restrictTime
              ? this.props.restrictTime.starttime
              : this.state.starthours,
            endhours: this.props.restrictTime
              ? this.props.restrictTime.starttime
              : this.state.starthours,
          ...
          this.setState(
            {
              startsession: "PM",
              starthours:
                this.props.restrictTime && this.props.restrictTime.endtime
                  ? Number(this.props.restrictTime.endtime) - 1
                  : this.state.starthours,
              startminutes: 59,
              endhours:
                this.props.restrictTime && this.props.restrictTime.endtime
          ...
          this.setState(
            {
              ...
              starthours:
                this.props.restrictTime && this.props.restrictTime.endtime
                  ? (Number(this.props.restrictTime.endtime) % 12) - 1
                  : this.state.starthours,
              startminutes: 59,
              endhours:
                this.props.restrictTime && this.props.restrictTime.endtime
      ...
      if (
        ...
        this.state.startsession === "PM"
      ) {
      } else if (
        this.props.restrictTime &&
        this.state.endsession === "AM" &&
        this.props.timemode &&
        parseInt(this.props.timemode) === 12
      ) {
        this.setState({
          returnedEndValue: true,
          endhours: (this.props.restrictTime.endtime % 12) - 1,
          endsession: "PM"
        });
      } else if (
        this.props.restrictTime &&
        this.state.endsession === "AM" &&
        this.state.starthours <= this.state.endhours
      ) {
        this.setState({
          returnedEndValue: true,
          endhours: this.props.restrictTime.endtime - 1,
          endsession: "PM"
        });
        ...
        this.setState(
          {
            returnedEndValue: true,
            endsession: this.state.endsession === "PM" ? "AM" : "PM"
          },
          this.propagateChange
        );
      }
    ...
    if (
      ...
      if (data === "start") {
        ...
        if (
          ...
          parseInt(this.props.timemode) === 12 &&
          this.state.startdate === this.state.enddate &&
          this.state.starthours > times.getHours() % 12
        ) {
          this.setState(
            {
              endhours: this.state.starthours,
          ...
          this.props.timemode &&
          parseInt(this.props.timemode) === 24 &&
          this.state.startdate === this.state.enddate &&
  ...
  onHandleUpPress = (data, part) => {
    if (data === "start" && part === "hours") {
      if (
        ...
        parseInt(this.props.timemode) === 24 &&
        this.props.restrictTime &&
        this.props.restrictTime.endtime - 1 <= this.state.starthours
      ) {
      } else if (
        this.props.restrictTime &&
        this.props.restrictTime.endtime - 1 <= this.state.starthours
      ...
      if (
        ...
        parseInt(this.props.timemode) === 24 &&
        this.props.restrictTime &&
        this.props.restrictTime.endtime <= this.state.endhours + 1
      ) {
        // console.log('blocked for above timemode 24 hr restriction');
      } else if (
        this.state.endsession === "PM" &&
        this.props.restrictToDayTime &&
        this.props.restrictTime &&
        this.props.restrictTime.endtime - 1 <= this.state.endhours
      ) {
      } else {
        this.setState(
          { endhours: this.state.endhours + 1, returnedEndValue: true },
          this.propagateChange
        );
      }
    } else if (data === "end" && part === "minutes") {
      this.setState(
        { endminutes: this.state.endminutes + 1, returnedEndValue: true },
        this.propagateChange
      );
    }
  };
  ...
  onHandleDownPress = (data, part) => {
    if (data === "start" && part === "hours") {
      if (
        this.props.restrictToDayTime &&
        this.state.startdate === this.state.enddate &&
        this.state.startsession === "AM" &&
        this.props.restrictTime &&
        this.state.starthours - 1 < this.props.restrictTime.starttime
      ) {
        // console.log('yes happen');
      } else if (
        this.props.restrictToDayTime &&
        this.props.restrictTime &&
        this.props.restrictTime.starttime >= this.state.starthours &&
        this.state.startsession === "AM"
      ) {
      ...
      if (
        ...
        this.props.restrictToDayTime &&
        this.props.restrictTime &&
        this.props.restrictTime.starttime >= this.state.endhours
      ) {
        // console.log('yes u got it');
      } else if (
        this.state.endsession === "PM" &&
        ...
        this.state.startsession === this.state.endsession &&
        this.state.starthours >= this.state.endhours
      ) {
      } else {
        this.setState(
      ...
      if (
        ...
        this.state.startsession === this.state.endsession &&
        this.state.starthours === this.state.endhours &&
        this.state.startminutes >= this.state.endminutes
      ) {
  ...
  WheelFunction = (item, events) => {
    if (item === "starthours" && events.nativeEvent.wheelDelta < 0) {
      this.state.starthours < parseInt(this.props.timemode) - 1 &&
        this.onHandleUpPress("start", "hours");
      ...
      this.state.endhours < parseInt(this.props.timemode) - 1 &&
        this.onHandleUpPress("end", "hours");
      ...
      this.state.startminutes >= 1 &&
        this.onHandleDownPress("start", "minutes");
    } else if (item === "endhours" && events.nativeEvent.wheelDelta > 0) {
  ...
  propagateChange = () => {
    ...
    if (parseInt(this.props.timemode) === 12) {
      this.setState({
        starttime: `${twoDigits(starthours)}:${twoDigits(
          startminutes
        )} ${startsession}`,
        endtime: endhours
          ? `${twoDigits(endhours)}:${twoDigits(endminutes)} ${endsession}`
          : null
      });
      ...
      this.setState({
        ...
        endtime: endhours
          ? `${twoDigits(endhours)}:${twoDigits(endminutes)}`
          : null
      });
      if (this.props.onChange != null) {
        const date = {
  ...
  render() {
    const { classes, defaultRange } = this.props;
        ...
        <Paper elevation={3} className={classes.paper_con}>
          ...
          <div className={classes.custombtn_style}>
            <Typography
              variant="body2"
              align="left"
              onClick={() => this.onTimePress("start")}
            >
              12:00 AM
            </Typography>
            <Typography
          ...
          <div className={classes.paper_con1}>
            <div
              ...
              onWheel={event => this.WheelFunction("starthours", event)}
            >
              ...
              <ExpandMoreIcon
                onClick={() =>
                  this.state.starthours >= 1 &&
                  this.onHandleDownPress("start", "hours")
                }
                className={classes.icon_style}
              />
            </div>{" "}
            ...
            <div
              className={classes.time_con}
              onWheel={event => this.WheelFunction("startminutes", event)}
            >
              <Typography align="center">Minutes</Typography>
              <ExpandLessIcon
                onClick={() =>
                  this.state.startminutes <= 58 &&
                  this.onHandleUpPress("start", "minutes")
                }
                className={classes.icon_style}
              />
              <Typography
            ...
            {parseInt(this.props.timemode) === 12 && (
              <div
                className={classes.time_con}
                onWheel={event => this.WheelFunction("startsession", event)}
              >
                <Typography align="center">AM/PM</Typography>
                <ExpandLessIcon
        ...
        {defaultRange && (
          <Paper elevation={3} className={classes.paper_con}>
            ...
            <div className={classes.paper_con1}>
              <div
                className={classes.time_con}
                onWheel={event => this.WheelFunction("endhours", event)}
              >
                <Typography align="center">Hours</Typography>
                <ExpandLessIcon
                  onClick={() =>
                ...
                <ExpandMoreIcon
                  onClick={() =>
                    this.state.endhours >= 1 &&
                    this.onHandleDownPress("end", "hours")
                  }
                  className={classes.icon_style}
                />
              </div>{" "}
              ...
              <div
                className={classes.time_con}
                onWheel={event => this.WheelFunction("endminutes", event)}
              >
                <Typography align="center">Minutes</Typography>
                <ExpandLessIcon
                  onClick={() =>
                    this.state.endminutes <= 58 &&
                    this.onHandleUpPress("end", "minutes")
                  }
                  className={classes.icon_style}
                />
                <Typography
                ...
                <ExpandMoreIcon
                  onClick={() =>
                    this.state.endminutes >= 1 &&
                    this.onHandleDownPress("end", "minutes")
                  }
                  className={classes.icon_style}
                />
              </div>
